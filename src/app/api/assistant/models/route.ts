 ...
 *
 * Proxied server-side (same reasons as the chat route — CORS + not sending
 * the key cross-origin from the browser). POST `{ baseUrl, apiKey }`, get back
 * `{ models: string[] }`. Best-effort: servers that don't expose `/models`
 * just return an empty array and the UI falls back to a free-text field.
 */
