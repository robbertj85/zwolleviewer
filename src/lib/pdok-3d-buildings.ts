// Per-building metadata for PDOK 3D Basisvoorziening and 3DBAG glb tiles
// (both generated by Tyler, same layout).
//
...
//
// PDOK meshopt-compresses some columns (e.g. the construction year), so
// those are decoded with the meshoptimizer decoder first.
...
function viewBytes(json: GlbJson, bin: Uint8Array, index: number): Uint8Array | null {
  ...
  if (!bv) return null;
  // Compressed columns point into the BIN chunk (buffer 0) through the
  // extension; `bv.buffer` is then only the uncompressed fallback.
  const meshopt = bv.extensions?.EXT_meshopt_compression;
  if (meshopt) {
    if (meshopt.buffer !== 0) return null;
    ...
    return target.subarray(0, bv.byteLength);
  }
  // Uncompressed metadata columns live in the BIN chunk (buffer 0).
  if (bv.buffer !== 0) return null;
 ...
 */
export async function embedBuildingMetadata(glb: ArrayBuffer): Promise<ArrayBuffer> {
  ...
  // PDOK spells it oorspronkelijkBouwjaar, 3DBAG oorspronkelijkbouwjaar.
  const yearProp =
    table?.properties?.oorspronkelijkBouwjaar ?? table?.properties?.oorspronkelijkbouwjaar;
  if (!table) return glb;
